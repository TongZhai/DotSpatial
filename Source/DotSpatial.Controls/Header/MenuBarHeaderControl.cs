...
namespace DotSpatial.Controls.Header
{
    ...
    public class MenuBarHeaderControl : HeaderControl
    {
        ...
        public override object Add(MenuContainerItem item)
        {
            ...
            if (root != null)
            {
                if (string.IsNullOrEmpty(item.MenuContainerKey))
                {
                ...
                {
                    var subs = item.MenuContainerKey.Split(new char[] { '_' });
                    var menuCollection = root.DropDownItems;
                    foreach (string sub in subs)
                    {
                        foreach (ToolStripMenuItem menuItem in menuCollection)
                        {
                            if (menuItem.Name == sub)
                            {
                                menuCollection = menuItem.DropDownItems;
                                break;
                            }
                        }
                    }
        ...
        public override object Add(SimpleActionItem item)
        {
            ...
            {
                if (item.MenuContainerKey.IndexOf("_") < 0)
                {
